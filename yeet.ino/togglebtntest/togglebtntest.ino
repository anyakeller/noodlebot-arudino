...
void setup() {
  currentToggle1Value = analogRead(togglePin1);
  currentToggle2Value = analogRead(togglePin2);
...
void loop() {
  ...
  if (currentToggle1Value > 900 && !Toggle1State){
    Toggle1State = true;
    Serial.print("Toggle 1 status is now: ");
    Serial.println(Toggle1State);
    Serial.println(currentToggle1Value);
  } else if (currentToggle1Value <20 && Toggle1State){
    Toggle1State = false;
    Serial.print("Toggle 1 status is now: ");
    Serial.println(Toggle1State);
    Serial.println(currentToggle1Value);
  }
  if (currentToggle2Value > 900 && !Toggle2State){
    Toggle2State = true;
    Serial.print("Toggle 2 status is now: ");
    Serial.println(Toggle2State);
    Serial.println(currentToggle2Value);
  } else if (currentToggle2Value < 20 &&Toggle2State){
    Toggle2State = false;
    Serial.print("Toggle 2 status is now: ");
    Serial.println(Toggle2State);
    Serial.println(currentToggle2Value);
